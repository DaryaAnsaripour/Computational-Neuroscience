...
class LIF(Behavior):
    def initialize(self, ng: NeuronGroup):
        ...
        self.tauD = self.parameter("tauD", )


        ng.threshold = ng.vector(mode=self.parameter("thresh"))
        ng.u = ng.vector(self.parameter("u_init", default="normal(-63, 5)"))
        ...
        ng.A = (1/ng.size)*torch.sum(ng.spike.float())
    ...
    def forward(self, ng):
        if self.refMode == "BLOCK_CURRENT":
            ...
            inp_u[refCondition] = 0
            ng.u += ((leakage + inp_u) / self.tau) * ng.network.dt
            ng.threshold += ((-(ng.threshold - self.threshold0) + self.theta * ng.spike.float()) / self.tauA)*ng.network.dt
